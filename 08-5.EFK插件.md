...
将下载的 kubernetes-server-linux-ppc64le.tar.gz 解压后，再解压其中的 kubernetes-src.tar.gz 文件。
...
``` bash
$ kubectl proxy --address='10.10.13.163' --port=8086 --accept-hosts='^*$'
Starting to serve on 10.10.13.163:8086
```

浏览器访问 URL：`http://10.10.13.163:8086/api/v1/namespaces/kube-system/services/kibana-logging/proxy`
