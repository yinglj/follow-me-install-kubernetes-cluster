...
``` bash
hostnamectl set-hostname power-k8s-01 # 将 power-k8s-01 替换为当前主机名
```
...
``` bash
cat >> /etc/hosts <<EOF
10.10.13.163 power-k8s-01
10.10.13.170 power-k8s-02
10.10.13.180 power-k8s-03
EOF
```
...
本操作只需要在 power-k8s-01 节点上进行，设置 root 账户可以无密码登录**所有节点**：

``` bash
ssh-keygen -t rsa 
ssh-copy-id root@power-k8s-01
ssh-copy-id root@power-k8s-02
ssh-copy-id root@power-k8s-03
```
