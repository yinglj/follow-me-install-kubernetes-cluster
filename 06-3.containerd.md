...
``` bash
cd /opt/k8s/work
wget -c https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.17.0/crictl-v1.17.0-linux-ppc64le.tar.gz
```
...
``` bash
cd /opt/k8s/work
tar -xvf crictl-v1.17.0-linux-ppc64le.tar.gz
```

下载runc源码，编译，复制后到/opt/k8s/work目录下

``` bash
cd /opt/k8s/work
wget -c https://github.com/opencontainers/runc/archive/v1.0.0-rc10.tar.gz
mkdir -p $GOPATH/src/github.com/opencontainers/runc
tar xvfz v1.0.0-rc10.tar.gz -C $GOPATH/src/github.com/opencontainers/runc --strip-components 1
yum install libseccomp libseccomp-devel -y
cd $GOPATH/src/github.com/opencontainers/runc
make
cp runc /opt/k8s/work
```
...
``` bash
cd /opt/k8s/work
wget -c https://github.com/containernetworking/plugins/releases/download/v0.8.5/cni-plugins-linux-ppc64le-v0.8.5.tgz
mkdir cni-plugins
sudo tar -xvf cni-plugins-linux-ppc64le-v0.8.5.tgz -C cni-plugins
```
下载containerd源码，编译，复制后到/opt/k8s/work目录下

``` bash
cd /opt/k8s/work
wget -c https://github.com/containerd/containerd/archive/v1.3.4.tar.gz
mkdir -p $GOPATH/src/github.com/containerd/containerd
tar xvfz v1.3.4.tar.gz -C $GOPATH/src/github.com/containerd/containerd --strip-components 1
yum install btrfs-progs-devel -y
make
mkdir -p /opt/k8s/work/containerd/bin
cp bin/* /opt/k8s/work/containerd/bin/
```
...
[plugins]
  [plugins."io.containerd.grpc.v1.cri"]
    sandbox_image = "registry.cn-beijing.aliyuncs.com/images_k8s/pause-ppc64le:3.1"
